...
import { products, site, whatsappLink, type Product } from "@/data/site";
import { ButtonLink, TextLink } from "@/components/ui/Button";
import { ArrowRight } from "@/components/ui/Icons";
import { Reveal } from "@/components/ui/Reveal";

const images: Record<Product["image"], StaticImageData> = { abacaxi, melancia, cocoSeco, cocoVerde };

const listHref = whatsappLink("Olá! Gostaria de receber a lista completa de produtos e a cotação do dia.");
const listExternal = Boolean(site.whatsapp);
    ...
    <section id="produtos" className="bg-white text-black">
      <div className="container-site py-14 sm:py-20 lg:py-24">
        ...
        <Reveal className="hidden items-end justify-between gap-8 sm:flex">
          ...
          <ButtonLink href={listHref} variant="outline-black" target={listExternal ? "_blank" : undefined} rel={listExternal ? "noopener" : undefined}>
        ...
        <ul
          className="no-scrollbar -mx-4 mt-8 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:mt-12 sm:grid sm:grid-cols-2 sm:gap-6 sm:overflow-visible sm:px-0 lg:grid-cols-4 lg:gap-5"
          aria-label="Produtos"
        >
          {products.map((p, i) => (
            <Reveal as="li" key={p.slug} delay={i * 70} className="w-[76vw] max-w-[340px] shrink-0 snap-start sm:w-auto sm:max-w-none">
